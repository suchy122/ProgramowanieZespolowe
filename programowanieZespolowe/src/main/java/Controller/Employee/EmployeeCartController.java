...
/**
 * The type Employee cart controller.
 */
public class EmployeeCartController implements Initializable {
    @FXML
    ...
    private void cartProduct() {
        try {
            ...
            while (rs.next()) {
                ...
                cena = cena - cena * 0.1;
                c.setProduct_price(cena);
                c.setProduct_description(rs.getString("Description"));
                suma += cena * rs.getInt("Quantity");
            ...
            cartSuma.setText("Kwota łącznie: " + suma + " zł");
...

                    {
                        ...
                        btn.setOnAction((ActionEvent event) -> {
                            try {
                                ...
                                st.setInt(2, EmployeeMainController.idUser);
                                ResultSet rs = st.executeQuery();
                                int quantity = 0;
                                while (rs.next()) {
        ...
        if (cartProductList.isEmpty()) {
            ...
            time = new Timeline(new KeyFrame(Duration.millis(1), new EventHandler<ActionEvent>() {
                ...
                public void handle(ActionEvent actionEvent) {
                    if (EmployeeOrderConfirm.refBool()) {
                        refresh();
                        time.stop();
                        EmployeeOrderConfirm.bool = false;
                    }
                }
            }));
            ...
            AnchorPane ap = FXMLLoader.load(Objects.requireNonNull(getClass().getResource("/FXML/Employee/OrderConfirm.fxml")));
    ...
    private void label() {
        userNameText.setText(String.valueOf(EmployeeMainController.userNameText2.getText()));
    }
}
